...
class Gaussian {
     ...
     */
     ...
     Gaussian(double weight, Eigen::MatrixXd mean, Eigen::MatrixXd cov) {
          ...
          */
          if (weight < 0) {
               std::cout << "Error! Weights must be non-negative.\n";
               throw;
          }
     ...
     double eval(Eigen::MatrixXd x) const {
          ...
          */
          double s = ( (x-m).transpose() * P.inverse() * (x-m) )(0,0);
          return w * exp(-0.5 * s * s) / sqrt(2 * M_PI) * sqrt(P.determinant());
     }
};
     ...
     */
     ...
     int predict(std::vector<Gaussian> birthMix = std::vector<Gaussian>()) {
          ...
          */ 
          ...
          size_t Jb {birthMix.size()};
          size_t Jc {mix.size()};
          size_t Js {w_spawn.size()};
     ...
     int prune() {
          ...
          on its weight. Merge any remaining components that are <U apart.
          ...
          */
          ...
          while (true) {
               ...
               for (auto& i : I) {
                    ...
                    double dist {(mDiff.transpose() * mix[i].P.inverse() * mDiff)(0,0)};
                    if (fabs(dist) <= U)
                         L.insert(i);
               }     
